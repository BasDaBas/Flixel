...
//TODO, an if else statement to check if the players collected all the cards and then show the GameOverState
//TODO, Fix if an enemy hits the player the enemie will flicker for a while so the player doesnt lose lifes to fast
//TODO: When 1 player has no lifes left, destroy player (getting error ATM)
//
 ...
 
{
	...
	private var _mWalls:FlxTilemap; //creates walls 
	private var _mFloor:FlxTilemap;
	private var _grpCards:FlxTypedGroup<Card>; //makes a FlxGroup in our NextLevelState to hold our Cards
	private var _grpEnemies:FlxTypedGroup<Enemy>; //makes a FlxGroup in our NextLevelState to hold our Enemies
	...
	{
		//setup the map
		_map = new FlxOgmoLoader(AssetPaths.room_005__oel);	// setup level
			
		_mWalls = _map.loadTilemap(AssetPaths.tilepath__png, 54, 54, "walls");	//setup tiles	
		...
		_mWalls.setTileProperties(3, FlxObject.ANY);	//set tile  to ANY collide	
		_mWalls.setTileProperties(4, FlxObject.NONE);	//set tile  to ANY collide	
		_mWalls.setTileProperties(5, FlxObject.ANY);	//set tile  to ANY collide	
		_mWalls.setTileProperties(6, FlxObject.ANY);	//set tile  to ANY collide	
		_mWalls.setTileProperties(7, FlxObject.ANY);	//set tile  to ANY collide	
		_mWalls.setTileProperties(8, FlxObject.ANY);	//set tile  to ANY collide	
		_mWalls.setTileProperties(9, FlxObject.ANY);	//set tile  to ANY collide	
		...
		_timeleft = Std.string(_tmrGame.timeLeft);
		...
		FlxG.sound.destroy;
		FlxG.sound.playMusic(AssetPaths.soundtracklevel3__wav,1,true);
		//setup cards
	...
	*/ 	
	...
	// set the speed of the player back to normal (250)
