 ...
 
{
	...
	{
		//setup the map
		_map = new FlxOgmoLoader(AssetPaths.room_006__oel);	// setup level
			
		_mWalls = _map.loadTilemap(AssetPaths.tilestest1__png, 54, 54, "walls");	//setup tiles	
		_mWalls.setTileProperties(1, FlxObject.NONE);	//set tile  to ANY collide
	...
	if (_cards == 21)
	{
		...
		_won = true;
		_ending = true;
		FlxG.camera.fade(FlxColor.BLACK, .33, false, completedGame);
	}
	...
	*/	
	...
	/**function that change the state to FinishedGame when called.
	* takes the var _cards to the GameOverState so it can be tracked there
	*/ 	
	private function completedGame():Void 	{
		
		FlxG.switchState(new FinishedGame(_cards));		
	}
